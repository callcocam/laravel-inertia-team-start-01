...
export default {
    ...
    init({ initialPage, resolveComponent, updatePage }) {
        ...
        } else if (window.sessionStorage.getItem('call.hardVisit')) {
            window.sessionStorage.removeItem('call.hardVisit')
    ...
    isCallResponse(response) {
        return response && response.headers['x-call']
    },
    ...
    visit(url, { method = 'get', data = {}, replace = false, preserveScroll = false, preserveState = false, only = [], headers = {}} = {}) {
        ...
        return Axios({
            ...
            headers: {
                ...
                'X-Call': true,
                ...(only.length ? {
                    'X-Call-Partial-Component': this.page.component,
                    'X-Call-Partial-Data': only.join(','),
                } : {}),
                ...(this.version ? { 'X-Call-Version': this.version } : {}),
            },
        }).then(response => {
            if (this.isCallResponse(response)) {
            ...
            if (Axios.isCancel(error)) {
                return
            } else if (error.response.status === 409 && error.response.headers['x-call-location']) {
                progress.stop()
                return this.hardVisit(true, error.response.headers['x-call-location'])
            } else if (this.isCallResponse(error.response)) {
    ...
    hardVisit(replace, url) {
        window.sessionStorage.setItem('call.hardVisit', true)
